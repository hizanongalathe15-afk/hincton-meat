...
type MaintenancePriority = 'checkout' | 'account' | 'orders' | 'browsing'

const statusTranslations: Record<'en' | 'sw', Record<string, string>> = {
  en: {
    sneakPeekTitle: 'What are we building?',
    sneakPeekHint: 'We’re adding new ways to shop smarter, faster, and more reliably.',
    priorityTitle: 'Notify me when the update is live',
    priorityHint: 'Tell us what matters most and we’ll email you first.',
    emergencyBypass: 'Are you a paying customer with an urgent issue? Click here for a temporary workaround.',
    waitTimeTitle: 'Bored? Here’s something useful while you wait.',
    statusTitle: 'Live status feed',
    snapshotTitle: 'Read-only snapshot',
    referralTitle: 'Thanks for your patience — enjoy this coupon.',
    helpText: 'Still need help? Use the link below to reach support directly.',
    loading: 'Sending request…',
    success: 'Thanks! We’ll email you as soon as we’re back.',
    error: 'Something went wrong. Try again in a moment.',
  },
  sw: {
    sneakPeekTitle: 'Tunaumba nini?',
    sneakPeekHint: 'Tunakuleta njia mpya za kununua kwa busara, haraka, na kwa uhakika zaidi.',
    priorityTitle: 'Tujulishe ukinirejesha mtandao',
    priorityHint: 'Tuambie ni jambo gani muhimu kuliko yote na tutakuandikia barua mwanzoni.',
    emergencyBypass: 'Je, wewe ni mteja mwenye malipo na una shida ya dharura? Bonyeza hapa kwa njia mbadala.',
    waitTimeTitle: 'Unaumwa kwa subira? Hili ni muhimu kwako sasa.',
    statusTitle: 'Mwongozo wa hali ya moja kwa moja',
    snapshotTitle: 'Toleo la kusoma tu',
    referralTitle: 'Asante kwa uvumilivu — pata punguzo hili.',
    helpText: 'Bado unahitaji msaada? Tumia kiungo hapa chini kufikia msaada moja kwa moja.',
    loading: 'Tuma ombi…',
    success: 'Asante! Tutakutumia barua pepe mara tu tutakaporudi.',
    error: 'Kitu kimeenda vibaya. Jaribu tena baadaye.',
  },
}
...
const generateReferralCouponCode = () => `HINCTON-${Math.random().toString(36).slice(2, 8).toUpperCase()}`

const getOrCreateReferralCouponCode = () => {
  const existing = getCookie('hincton_maintenance_coupon')
  if (existing) return existing
  const code = generateReferralCouponCode()
  setCookie('hincton_maintenance_coupon', code, 2)
  return code
}
