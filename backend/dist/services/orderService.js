...
class OrderService {
    async createOrder(orderData) {
        try {
            ...
            // Process payment
            const paymentResult = await paymentService_1.paymentService.createPayment({
                orderId: order.id,
                userId: orderData.userId,
                amount: total,
                currency: 'KES',
    ...
    async cancelOrder(orderId, reason, userId) {
        try {
            ...
            // Restore stock
            await this.updateStockAfterOrder(order.orderItems, 'increase');
            ...
            if (order.paymentStatus === 'PAID') {
                ...
                if (payment) {
                    await paymentService_1.paymentService.processRefund({
    ...
    async getUserOrders(userId, page = 1, limit = 20, filters) {
    ...
    async updateStockAfterOrder(items, operation) {
        for (const item of items) {
    ...
    async getOrderStats(dateRange) {
        ...
        const [totalOrders, revenueData, statusData, paymentData] = await Promise.all([
            database_1.prisma.order.count({ where }),
