...
class CategoryService {
    ...
    async getCategory(categoryId, includeProducts = false) {
        try {
            const category = await database_1.prisma.category.findUnique({
                where: { id: categoryId },
                include: {
    ...
    async getCategories(page = 1, limit = 50, filters, sortBy = 'sortOrder', sortOrder = 'asc') {
        try {
    ...
    async getCategoryStats() {
        try {
            const [totalCategories, activeCategories, featuredCategories, categoryData] = await Promise.all([
                database_1.prisma.category.count(),
                database_1.prisma.category.count({ where: { isActive: true } }),
                database_1.prisma.category.count({ where: { isFeatured: true } }),
                database_1.prisma.category.groupBy({
                    by: ['id'],
                    include: {
                        products: {
                            where: { isPublished: true }
            ...
            const categoriesWithProducts = categoryData.filter(cat => cat.products && cat.products.length > 0).length;
