...
exports.getConversationMessages = exports.getConversations = void 0;
...
exports.getConversations = (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    const messages = await database_1.prisma.liveChatMessage.findMany({
        where: { userId },
        select: {
            id: true,
            roomId: true,
    ...
    for (const msg of messages) {
        if (!byRoom.has(msg.roomId)) {
            ...
            byRoom.set(msg.roomId, {
                ...
                participantName: otherParticipant?.profile?.fullName ??
...
exports.getConversationMessages = (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    const exists = await database_1.prisma.liveChatMessage.findFirst({
        where: { roomId: conversationId, userId },
        select: { id: true }
    });
    if (!exists) {
        throw new middleware_1.NotFoundError('Conversation', conversationId);
    }
    const messages = await database_1.prisma.liveChatMessage.findMany({
        where: { roomId: conversationId, userId },
        select: {
