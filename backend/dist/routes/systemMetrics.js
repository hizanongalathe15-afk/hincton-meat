...
router.get('/metrics', async (req, res) => {
    try {
        const metrics = await systemMetricsService_1.systemMetricsService.getSystemMetrics();
...
router.get('/wake-time', async (req, res) => {
    try {
        const wakeTime = systemMetricsService_1.systemMetricsService.getSystemWakeTime();
        res.json({
            success: true,
            wakeTime,
            generatedAt: new Date().toISOString()
...
router.get('/metrics/history', async (req, res) => {
    try {
        ...
        let metrics = [];
        try {
            metrics = await prisma_1.prisma.systemMetric.findMany({
                where: {
