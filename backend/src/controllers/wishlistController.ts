...
export const checkWishlistStatus = async (req: AuthRequest, res: Response, next: any) => {
  try {
    const userId = req.user?.id
    ...
    if (!wishlist) {
      const wishlistStatus = productIds.map((productId: string) => ({
        productId,
        inWishlist: false
      }))
...
export const clearWishlist = async (req: AuthRequest, res: Response, next: any) => {
  try {
    const userId = req.user?.id
...
export const moveWishlistToCart = async (req: AuthRequest, res: Response, next: any) => {
  try {
    const userId = req.user?.id
    const { productIds } = req.body
