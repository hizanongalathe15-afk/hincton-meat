...
class QrCodeService {
   ...
   */
  async create(data: CreateQrCodeInput) {
   ...
   */
  private async generateQrImageDataUrl(code: string): Promise<string> {
   ...
   * Update QR code
   */
  async update(id: string, data: UpdateQrCodeInput) {
    const qrCode = await prisma.qrCode.findUnique({ where: { id } });
    if (!qrCode) throw new Error('QR code not found');
   ...
   */
  async trackScan(code: string, scanData: {
    ipAddress?: string;
    userAgent?: string;
    referrer?: string;
    userId?: string;
   ...
   */
  async getStats(id: string, period: 'day' | 'week' | 'month' | 'year' = 'month') {
    const qrCode = await prisma.qrCode.findFirst({
      where: { id, deletedAt: null },
    });
   ...
   */
  async markConverted(code: string, sessionId?: string, userId?: string) {
    ...
    const where: any = { qrCodeId: qrCode.id, converted: false };
